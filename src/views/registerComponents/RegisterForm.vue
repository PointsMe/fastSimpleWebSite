...
<template>
  <div class="login-form">
    <el-form :key="formId" :model="formModel" ref="formModelRef" :rules="formRules">
      <el-row :gutter="12">
        <el-col
    ...
    <div class="login-last">
      ...
      <div
        :class="
          props.step === '1' ? 'btn-login margin-top-60' : 'btn-login margin-top-148'
        "
      >
        <el-button
          class="el-btn-color"
...
import { emailFormStep1, phoneFormStep1, formStep2 } from "./formList";
...
const formRules = computed(() => {
  return {
    ...
    emailAccount: [
      {
        required: true,
        message: i18n.global.t("aboutLogin.pleaseInputEmail"),
        trigger: "blur",
      },
    ],
    verificationCode: [
      {
        required: true,
        message: i18n.global.t("aboutLogin.pleaseInputCode"),
        trigger: "blur",
      },
    ...
    subName: [
      {
        required: true,
        message: i18n.global.t("aboutLogin.componeyName"),
        trigger: "blur",
      },
    ...
    vatNumber: [
      {
        required: true,
        message: i18n.global.t("aboutLogin.componeyPI"),
        trigger: "blur",
      },
      // { min: 3, max: 5, message: 'Length should be 3 to 5', trigger: 'blur' },
    ],
    ...
    countryId: [
      {
        required: true,
        message: i18n.global.t("aboutLogin.countryName"),
        trigger: "change",
      },
    ],
    provinceId: [
      {
        required: true,
        message: i18n.global.t("aboutLogin.printent"),
        trigger: "change",
      },
    ],
    city: [
    ...
    if (valid) {
      if (props.step === "1") {
        ...
        const params = {
          name: formModel.name,
          account:
            props.registerStyle === "1"
              ? `${countryCode.value.replace("+", "")}-${formModel.phoneAccount}`
              : `${formModel.emailAccount}${emailCode.value}`,
